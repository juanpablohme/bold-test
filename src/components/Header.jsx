...
const Header = () => {
  ...
  return (
    <header>
      ...
      <div 
        className={`menu-overlay ${ toggleMenu ? 'opened' : 'closed' }`}
        onClick={ () => setToggleMenu(false) }
      ></div>

      <nav className={ toggleMenu ? 'opened' : 'closed' }>
        <span 
          className='close-menu material-icons-outlined'
          onClick={ () => setToggleMenu(false) }
        >
          close
        </span>
        <ul>
          <li><a href='/'  onClick={ () => setToggleMenu(false) }>Mi negocio</a></li>
          <li><a href='/'  onClick={ () => setToggleMenu(false) }>Ayuda <span>?</span></a></li>
        </ul>
      </nav>
    </header>
  )
